...
OpMode.prototype.run = function (cb) {
    ...
    var done = function (err) {
        ...
        if (err) {
            ...
            // The replacement process has been spawned. It is not joinable yet: this is the signal
            // for the notifier to start waiting for the new instance to come online, not the old
            // one, which was still up and still polling at run-started.
            self.emit('run-restarted', opServer);
        }
