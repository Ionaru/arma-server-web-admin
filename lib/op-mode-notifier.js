...
var OpModeNotifier = function (opMode, discord, options) {
    ...
    // The in-flight run, if any: {server, startedAt, done, cancel}. done gives exactly one terminal
    // message per run; cancel abandons a still-pending online-wait when a newer run supersedes it.
    this.current = null;
    ...
    opMode.on('run-restarted', function (opServer) {
        self.onRunRestarted(opServer);
    });
...
// Marks the in-flight run finished and abandons any online-wait it armed, without emitting a
// message. Used when a run fails, or when a newer run supersedes an older one whose server had not
// yet come back online (running clears at process spawn, minutes before joinable).
OpModeNotifier.prototype.cancelCurrent = function () {
    if (!this.current) {
        return;
    }
...
OpModeNotifier.prototype.onRunStarted = function (opServer) {
    ...
    this.cancelCurrent();
    this.current = {server: opServer, startedAt: Date.now(), done: false, cancel: null};

    this.post('🔄 Restarting **' + opServer.title + '** for the operation.');
};

// The replacement process has now been spawned. Only here is it meaningful to wait for the server
// to come online: before this the op server is still the old process, still answering gamedig, so a
// wait armed at run-started would resolve against the wrong instance (a false "back online").
OpModeNotifier.prototype.onRunRestarted = function (opServer) {
    var run = this.current;

    if (!run || run.server !== opServer || run.done) {
        return;
    }

    var self = this;

    run.cancel = opServer.waitUntilOnline(this.onlineTimeoutMs, function (err) {
        if (run.done) {
            return;
        }
        ...
        if (!err) {
            ...
            return self.post('✅ **' + opServer.title + '** is back online (took ' + took + ').');
        }

        if (err.reason === 'offline') {
            return self.post('⚠️ **' + opServer.title +
                '** started but went offline again before coming online.');
        }

        self.post('⚠️ **' + opServer.title + '** has not come back online after ' +
...
OpModeNotifier.prototype.onRunFailed = function (opServer, err) {
    ...
    // Abandon any pending online-wait for this run: only the failure message should go out.
    this.cancelCurrent();
